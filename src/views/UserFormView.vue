...
import { TOAST_STATUS, useToast } from '@/hooks/useToast.ts';
...
// Sprawdzanie czy jesteśmy w trybie edycji
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      await updateUser({ id: route.params.id }, form.value).then(() => addToast('Użytkownik został zaktualizowany'));
    } else {
      await createUser({}, form.value).then(() => addToast('Dodano nowego użytkownika'));
    }
  } catch (err) {
    addToast('Wystąpił błąd. Spróbuj ponownie', TOAST_STATUS.ERROR);
    console.error(err);
  }
}
...
const handleImageError = (e: Event) => {
  ...
  img.src = '/placeholder-avatar.png' // Zastąp domyślnym avatarem
...
onMounted(async () => {
  if (isEditMode.value) {
    try {
      ...
      addToast('Nie udało się pobrać użytkownika', TOAST_STATUS.ERROR);
      console.error(err);
    }
  }
});
